 ...
 * `video_downloader.py`: Sample code demonstrating how to download data samples. Supports resume: uses the latest log file to skip processed videos and skips files that already exist.

 * `WLASL-data.ipynb`: Jupyter notebook for data exploration, gloss counts, category classification, and subset creation.

 * `gloss_video_counts.json`: Gloss → video count mapping (generated from `WLASL_v0.3.json`).

 * `gloss_categories.json`: Gloss → category mapping (generated via OpenAI API classification).

 * `WLASL_subset.json`: Subset of `WLASL_v0.3.json` for selected glosses (generated by the notebook).
...
Data Exploration & Subset Creation (NUS-ISS)
-----------------
This section describes the data exploration workflow and tools for creating custom WLASL subsets.

**Video downloader resume behavior**

The `video_downloader.py` supports stop-and-restart: it will continue downloading without re-downloading existing files. It uses the latest `.log` file (by modification time) to skip already-processed videos and checks `os.path.exists()` for each file before downloading.

**Derived files**

| File | Description |
|------|-------------|
| `gloss_video_counts.json` | Maps each gloss to its video count (from `WLASL_v0.3.json`) |
| `gloss_categories.json` | Maps each gloss to a semantic category (e.g. `objects`, `food_drink`, `people_family`) via LLM classification |
| `WLASL_subset.json` | Subset of `WLASL_v0.3.json` containing only selected glosses |

**Workflow (see `WLASL-data.ipynb`)**

1. **Gloss lookup** – Find a gloss index and remaining count in the dataset.
2. **Count videos per gloss** – Build `gloss_video_counts.json` from `WLASL_v0.3.json`.
3. **Classify glosses** – Use OpenAI API to assign each gloss to a category; save to `gloss_categories.json`.
4. **Count glosses per category** – Summarise how many glosses fall into each category.
5. **Create subset by selected glosses**:
   - Pick a category and list glosses with video counts.
   - Choose specific glosses from that list.
   - Build `WLASL_subset.json` containing only those glosses (same structure as `WLASL_v0.3.json`).

**Categories** (used in `gloss_categories.json`): `people_family`, `animals`, `food_drink`, `actions_verbs`, `objects`, `colors`, `places`, `time_numbers`, `emotions_adjectives`, `body_health`, `clothing`, `sports_activities`, `abstract_concepts`, `greetings_social`, `other`.
